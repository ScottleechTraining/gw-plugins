---
description: Daily morning digest â€” synthesize the night's pipeline output into a "what to dig into first" briefing
---

# /gw-morning-digest â€” Daily Action-Oriented Briefing
...
1. `D:\Claude Projects\Gridiron Warrior\_morning-briefing.md` â€” markdown source-of-truth, archived to git
2. `D:\Claude Projects\Gridiron Warrior\_morning-briefing.html` â€” phone-optimized HTML used by the email sender
3. `D:\Claude Projects\Gridiron Warrior\_dashboard-index.html` â€” self-contained dashboard page (4-panel dropdown) that the `build-gw-dashboard.ps1` script deploys to Netlify each morning
...
Write the markdown file using this exact structure. Keep it tight â€” every section earns its spot.

```markdown
Subject: GW Daily â€” YYYY-MM-DD â€” <3-word essence of today's move>

# GW Daily â€” YYYY-MM-DD
...
Example: **Run** `/gw-content-forge "1,808 saved posts â€” saving vs doing"` to expand Angle 1 from today's seed.
...
One line per source. Skip lines for sources with zero new items. Lead with the count, then the standout. Dewey / voice / screenshot counts come from `wiki/log.md` only â€” do NOT open those folders.
...
- (etc â€” only include sections that actually have content today)
```
...
- Doctype HTML4 transitional, table-based layout (NOT divs/flex â€” clients strip them)
...
- NO tables for data (single-column lists only â€” multi-column tables overflow on phone)
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f5f5f5;padding:20px 0;">
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#ffffff;border-radius:8px;padding:24px;">
      <tr><td>
        <p style="font-size:12px;color:#888;margin:0 0 4px 0;text-transform:uppercase;letter-spacing:1px;">GW Daily â€” YYYY-MM-DD</p>
...
This is the self-contained file deployed to Netlify each morning by `build-gw-dashboard.ps1`. ONE HTML document. Phone-first **editorial sports tabloid** aesthetic â€” think SI 1987 + locker room + Coach Leech voice. Confident, tactile, magazine-issue energy.

**DO NOT regress this to generic SaaS dashboard styling** (light gray bg + white card + system fonts + plain dropdown). The bold design is intentional and on-brand. If you find yourself reaching for `-apple-system` or `border-radius: 8px` on the card, stop â€” the spec below is the design.
...
- Color tokens (CSS variables â€” use exactly these in `:root`):
  - `--ink: #0a0a0a` (near-black for bars, headlines, primary text)
  - `--paper: #f1ece1` (newsprint cream page bg)
  - `--paper-deep: #e6dfd0` (shadow tone)
  - `--rust: #c8102e` (URI red â€” the command color, used sparingly for impact)
...
- **Layout (three structural blocks plus a footer):**
  - `<header class="masthead">` â€” full-width ink-black bar with 6px URI-red bottom border. Inside (max-width 760px centered): brand mark "Gridiron Warrior" in Oswald 700 caps letter-spaced, "Command" sub-label in Oswald 300 in URI red. Right side: "Issue" label + date as `YYYY.MM.DD` (dots, not dashes) in Oswald 700.
  - `<nav class="tabs">` â€” full-width ink-black bar with 4 button tabs flush across. Active tab gets URI red bg with cream text and weight 700. Tabs use Oswald 500/700 caps, letter-spacing 0.18em. **Tab labels: "Today" / "AI" / "Business" / "S&C"**. NOT a `<select>` dropdown â€” this is a tab bar.
  - `<main class="paper">` â€” max-width 760px centered container. Inside: four `<section class="panel">` blocks, only `.active` visible.
  - `<footer class="colophon">` â€” tiny letterspaced Oswald caps, "Keep the Fire Burning" in URI red, then "Leech Â· Gridiron Warrior" in muted gray.
...
Skeleton template (use this exactly â€” replace YYYY.MM.DD with the dotted date for masthead, YYYY-MM-DD with the standard date in file paths, and the content inside each `<section class="panel">`):
...
<title>GW Command â€” YYYY-MM-DD</title>
...
## Read budget (HARD CAPS â€” do not exceed)
...
- **Do NOT read voice notes, Dewey notes, or screenshot notes.** Pull their counts and top hits from `wiki/log.md` â€” `/gw-voice-ingest`, `/gw-dewey-daily`, and `/gw-screenshot-ingest` already wrote one-line summaries there. That's the digest's source of truth for those sources.
- **CLAUDE.md: already in your project context â€” do NOT re-Read it.**
...
- If a daily brief file is missing (AI / Business / S&C â€” pipeline failure or pre-launch), the corresponding dashboard panel must still render with "No brief produced today. Check pipeline logs." so the dashboard never breaks.
