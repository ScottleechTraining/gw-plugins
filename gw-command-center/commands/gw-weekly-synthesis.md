...
```bash
git -C "C:\Claude Projects" status --porcelain -- "Gridiron Warrior/wiki/" ":(exclude)Gridiron Warrior/wiki/log.md" ":(exclude)Gridiron Warrior/wiki/system/wiki-health-report.md" ":(exclude)Gridiron Warrior/wiki/system/wiki-health-history.md"
```

(Three machine-written files are excluded on purpose. `log.md` is append-only and the daily jobs legitimately dirty it every day — it blocked the 2026-07-12 run for nothing. `wiki-health-report.md` and `wiki-health-history.md` are regenerated by `scripts/wiki_health.py --write`, which is step 1 of this job's own contract, so the job dirtied the tree it then refused to work in — that blocked the 2026-08-16 run. The nightly closeout commits all three.)
