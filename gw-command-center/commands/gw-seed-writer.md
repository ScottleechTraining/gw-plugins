...
- A required teaching ingredient the sources do not hold (the second alternative for a comparison, the criteria for a decision guide, the steps of a worked example, the checks for a card) gets its own bullet starting with `Needs:`. The seed says "we have the decision criteria" or "someone must supply the criteria"; it never implies the second when the first is true. When the gap belongs to a coach-demand signal, cite the signal id in the same bullet so the gap report can join it. A `Needs:` bullet with no durable id is reported as unanchored. Rules: the plugin's `ig-carousel/references/teaching-readiness.md` (F4).
