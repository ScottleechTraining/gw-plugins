...
**Version:** 0.31.1 (feeder F3 to F6 with Scott's decisions applied: v1 freebie rule live, cutover 2026-09-17, 2026-09-10)
...
| **0.22.0** (2026-09-09) | **Save-led reference module (roadmap idea 8).** New reference `ig-carousel/references/save-reference-slides.md` (Codex editorial packet: reference forms, placement, eight preflight tests, business boundary; three worked drafts appended as unrendered, unapproved examples whose sources already have shipped packs). Wired into `gw-content-forge` (loaded after the brief and before copy for save-led variants; each save-led variant needs its own supported payoff), `ig-carousel/SKILL.md` (opted-in mode plus a compliance-checklist item for the card and phone readability), `gw-carousel-batch` (new save-led builds and explicit `action: save` rewrites; REPAIR NEEDED report when the source cannot support a card; style-only rebuilds never retrofit), and `copy-record.md` (one pointer; no schema change). Failure path made operational: Forge-chosen save moves to the next feasible action with the reason recorded; Scott-requested save stops for editorial repair. Tested on two live topics (see roadmap "Idea 8 Test Record"): save-led new build (`sort-him-before-the-first-whistle`, Forge chose save on its own, five-item checklist card as body slide s05, Editorial Long-Form, browser edit then restyle to Paper Minimal with the record byte-identical) and an explicit conversation-to-save reroll (`your-heavy-core-circuit-is-missing-the-active-ingredient`, Newsprint Bauhaus control deck with no card, then `action: save` reroll that rewrote brief, body, card at s06, caption, and CTA together with deck/variant/source identity preserved and every card number traced to the concept page). Both topics review-ready in `_inbox` with fresh tokens; stale-token SHIP blocked; legacy decks untouched; nothing published. Usefulness hypothesis only; no save-rate or revenue claim. |

| **0.23.0** (2026-09-09) | **Share-led module (roadmap idea 9).** New reference `ig-carousel/references/share-led-decks.md` (Codex packet: the job, automatic brief mapped into existing fields, writing rules, five pass/fail checks; three worked drafts appended as the share alternatives to the idea 8 drafts, never to be enqueued alongside them). Wired into `gw-content-forge` (loaded after the brief for share-led variants), `ig-carousel/SKILL.md` (opted-in mode plus a checklist item), `gw-carousel-batch` (new share-led builds and explicit `action: share` rerolls; REPAIR NEEDED when no recipient or decision exists; existing share decks not retrofitted), and `copy-record.md` (one pointer; no schema change). Tested: explicit save-to-share reroll of `sort-him-before-the-first-whistle` (identity preserved, five of six slides plus caption and CTA rewritten, checklist card replaced because it served a solo task) and a new share-led build (`you-and-your-kicking-game-coach-are-grading-the-same-period-differently`, Forge landed share with the special teams coordinator as recipient and a day-plus-rotation joint decision in the existing brief fields, The Case build, five checks passed, no tag bait). Three test topics left review-ready in `_inbox`; stale-token SHIP blocked; nothing published. Usefulness hypothesis only; no share-rate claim. |

| **0.24.0** (2026-09-09) | **Rhetorical approaches module (roadmap idea 5).** New reference `ig-carousel/references/rhetorical-approaches.md` (Codex packet: six teaching progressions with use-when, body progression, and avoid rules; automatic selection after the action; rewrite test; personal-voice boundary; six worked outlines appended as examples on an already-shipped source). Wired into `gw-content-forge` (choose the progression per variant before drafting, record `approach: <name>; <reason>` at the head of `brief.reason`, variants differ in progression, tie-break on at most six recent managed decks or skip), `ig-carousel/SKILL.md` (opted-in mode plus a checklist item), `gw-carousel-batch` (action rerolls may change the approach; style-only never), and `copy-record.md` (the reason field convention; no schema change). Tested: one Forge run on `trainable-menu` (topic `your-trainer-said-he-can-do-indy-so-you-pulled-him-out-of-indy`) produced Carousel #1 as a save-led Decision guide (choice, distinguishing question, conditions, next step, decision card at s04) and Carousel #2 as a share-led Observation (scene, overlooked detail, significance, practical response), both reason fields opening with `approach:`; the tie-break found no approach history on the four managed decks and proceeded without a question; Personal lesson was considered and rejected because the transcript supplies a rule, not an arc, and no first-person memory was invented; The Case build, review-ready in `_inbox`, stale-token SHIP blocked. No engagement claim. |

| **0.25.0** (2026-09-09) | **Visual teaching module (roadmap idea 4).** New reference `ig-carousel/references/visual-teaching.md` (Codex prototype packet: selection rules for comparison, event sequence, and annotated example; layout, saved-copy, and preflight rules; provenance of the three proofs; the proofs' geometry as CSS/HTML appendix to be translated into the selected pack, never shipped as Oswald or the document palette). Wired into `gw-content-forge` (at most one visual body slide per deck, only when a source-supported relationship earns it, recorded as `visual:` in `brief.reason`, every label its own field), `ig-carousel/SKILL.md` (opted-in mode plus a checklist item), `gw-carousel-batch` (build per the reference; style-only rebuilds keep text and meaning or keep the arrangement inside the new style), and `copy-record.md` (unique bindings for repeated labels; no new role). Tested: one Forge run on `exercise-order-within-session` (topic `you-cut-to-four-exercises-and-still-ran-out-of-period`) chose a save-led Comparison with a `visual: comparison` body slide at s03 carrying 13 separately bound fields (two identical rest lines as separate bindings, no numbers beyond the A1/A2 labels) and rejected a sequence as prose in rectangles; Editorial Long-Form build with a rail-and-stop versus bracket-and-arrow loop cue; a label on the visual slide edited through the real save path and round-tripped with nothing else changed; restyle to Newsprint Bauhaus preserved the record byte for byte and carried both panels, the order, the loop cue, and the non-color distinction; review-ready in `_inbox`, stale-token SHIP blocked; nothing published. No engagement claim. |

| **0.25.1** (2026-09-09) | **Polish re-trigger gap closed.** Runtime: `apply_review` now stamps `polish_at` when it records a POLISH note and clears it on SHIP; `scan_folders` carries the stamp; new `scripts/gwqueue/polish_pending.py` lists each noted topic as PENDING or REBUILT by comparing the carousel HTML modified time to the stamp (no stamp, no HTML, or older HTML means PENDING). `gw-carousel-batch` discovery runs that helper before filling the restyle, cover, and action buckets and rebuilds only PENDING rows; REBUILT rows are reported as awaiting review. Before this, every noted topic was rebuilt every night until Scott decided it, which re-rolled managed decks and could overwrite a same-day browser edit. 185 queue tests pass. |

| **0.25.2** (2026-09-09) | **Edit digest feeds skill tune (roadmap idea 6, first honest form).** New runtime `scripts/gwqueue/edit_digest.py` walks each managed deck's history (initial `carousel-build.json`, content-hash backups, live master), diffs the embedded records in time order, and labels each step `edit` (Scott changed words: action unchanged, few slides touched) or `reroll` (system rewrite for a new action). Writes `Deliverables/_system/edit-digest-<date>.md` with before/after pairs by slide field, caption, and CTA. `gw-skill-tune` ANALYZE now harvests it as a fifth signal row. Post metrics stay in the pilot ledger by hand. Also this day, outside the plugin: idea 10 `scripts/gwqueue/dewey_structures.py` (heuristic shape index over the Dewey saves, quarantined rows, summary at `wiki/business/dewey-teaching-structures.md`). |
| **0.26.0** (2026-09-09) | **Feeder F1: preserve the idea handoff.** Runtime (vault): new `scripts/gwqueue/idea_context.py` and contract `scripts/gwqueue/IDEA-CONTEXT.md`. `backfill_forge_ideas.extract_entries` now parses each seed angle as the block between its `### Angle` heading and the next heading, replacing the 600-character lookback that tagged the 2026-09-07 receiver angle (declared IG carousel) and the 2026-09-08 baseline angle (declared Twitter thread) as `email` via the morning-briefing harvest. New rows carry an optional versioned `idea_context` (idea id, revision, origin path/date/locator, source hash, declared format, source refs with quarantine labels, body sketch, CTA proposal, confidence, completeness, warnings; bounded at 10 refs, 8 bullets, 12,000 chars). Merge never overwrites: revision conflicts, slug collisions, format/score mismatches, and invalid records print as `CONTEXT` diagnostics. `append_forge_ideas --from-report` passes an explicit origin and date. `nightly_forge_pick` renders a valid record as an indented data-only blockquote under the pick; eligibility, ranking, and the novelty gate are unchanged. Commands: `gw-seed-writer` names the labels as the harvest contract; `gw-nightly-forge` step 1.6 treats context as proposals and step 3.7 adds `Idea origin:` to the receipt; `gw-content-forge` receipt template carries the line. Legacy rows untouched; no schedule change. 232 queue tests pass (26 new). Isolated rehearsal on a queue copy: both fixtures correct, selection order identical, live file untouched. First real overnight cycle not yet observed. |
| **0.27.0** (2026-09-09) | **Feeder F2: prevent unsupported claim escalation.** New shared reference `ig-carousel/references/claim-boundaries.md` (six claim categories, ten verification rules, check before the hook and after drafting, repair ladder narrow > attribute > label > remove > stop, overnight limit of three extra sources and one recheck, the `claim-check.json` receipt, blocked-topic procedure, worked examples from the audit's receiver-deck escalation). `gw-content-forge` gains Step 2B.1a (claim register before the hook) and Step 3.7 (claim gate on every asset before save, `python -m scripts.gwqueue.claim_check stamp`), and PULLED FROM THE BRAIN ends with a `Claim check:` line. `gw-nightly-forge` step 2.5: budget, and blocked topics go to `_pending-drafts/claim-blocked/<slug>/` with the backlog row `skipped` under a `claim-blocked (F2):` reason so a created folder never retires an unfinished idea. `gw-seed-writer` sorts the hook's claim before scoring and prefixes application and illustrative bullets (no new label, no score change). `gw-carousel-batch` reports `claims STALE` on action rerolls. `copy-record.md` Claim Coverage section; one pointer each in rhetorical, visual, and save references. Runtime (vault): `scripts/gwqueue/claim_check.py` validates the sidecar, recomputes derived arithmetic, stamps pack and copy-text hashes, and reports none/current/stale/needs-review/invalid; the review page shows the badge, never blocks SHIP. 245 queue tests pass (13 new). Rehearsal: the real receiver deck repaired to narrowed and removed claims with one needs-review; a supported deck; a blocked synthetic hook not retired. No second model call, no new queue status, no migration, no approval invalidated. First real overnight cycle not yet observed. |
| **0.28.0** (2026-09-10) | **Feeder F3: one grading sheet, gw-idea-quality-v1.** New reference `ig-carousel/references/idea-quality.md` (five gates then five 0-4 dimensions: coach relevance, decision specificity, reader payoff, source support, moment of use; qualification 15+ with every dimension 2+; action, business connection, and effort recorded outside the total). `gw-seed-writer` scores with it in the same pass (the old revenue/voice/urgency/ease block is replaced; the `**Total: N/20**` line stays parser-compatible; repair candidates at 11 to 14 get no Forge command). `_fable/playbooks/content-seed-rubric.md` becomes a pointer. Runtime (vault): `scripts/gwqueue/idea_quality.py` parses the seed block into an optional `idea_assessment` on the backlog row, validates ranges, reasons, and arithmetic, ties it to the F1 context revision, and ranks; `nightly_forge_pick` runs shadow lines by default and a whole-run legacy fallback under Scott's `Deliverables/_system/idea-quality-settings.json` switch whenever any eligible row lacks a valid current assessment (no mixed ranking); the ideas page labels every score `v1` or `legacy`. `gw-nightly-forge` holds freebies on v1 picks until Scott approves a v1 rule. Calibration set of twelve real ideas: `docs/superpowers/2026-09-10-feeder-f3-calibration.md`, awaiting Scott. Dashboard score chip stays unlabeled (manual deploy, not done). No rescoring of history. |
| **0.29.0** (2026-09-10) | **Feeder F4: teaching readiness before slides.** New reference `ig-carousel/references/teaching-readiness.md` (ingredient checklist per teaching option, three outcomes, where the check runs, a two-document bounded lookup, THE MESSAGE receipt lines, the needs-material repair path). `gw-content-forge` runs the check per variant after action, approach, and visual and before slides; THE MESSAGE template carries readiness, payoff, ingredients, rejected option, and selected approach; Step 3.6 gains test 6 (payoff delivery). `gw-nightly-forge` sends needs-material topics to `_pending-drafts/needs-material/<slug>/` with the row `skipped` under a `needs-material (F4):` reason. `gw-seed-writer` marks a missing ingredient with a `Needs:` bullet. Runtime: the retirement test now covers both pending-drafts buckets. No new JSON master. |
| **0.30.0** (2026-09-10) | **Feeder F5: coach-demand lane, prepared, not yet fed.** `gw-seed-writer` Step 1a lane 2 reads at most three approved records from `wiki/business/coach-demand-signals.md` via `python -m scripts.gwqueue.demand_signals list --limit 3`; a signal is question evidence only, cited as `[[business/coach-demand-signals#<id>]]`, never a testimonial or a voice source. `gw-content-forge` 2B.1a and `gw-nightly-forge` 1.6 say the same. Runtime: `scripts/gwqueue/demand_signals.py` validates records (kinds, statuses, no names, handles, emails, URLs, or paths in writer-visible fields), lists available ones, and finds backlog rows linked to a withdrawn signal. The library page ships with its schema and zero records. Source register and extraction guide at `docs/superpowers/feeder-f5/`; extraction waits on Scott's approval of source classes C, F, and D. No connector, no scraping, no recurring job. |
| **0.31.0** (2026-09-10) | **Feeder F6: archive lane.** New reference `ig-carousel/references/archive-retrieval.md` (when to look back, budget of one question, three entries, two documents per run; retrieval and currentness steps; four outcomes; the five-line receipt). New wiki page `business/evergreen-teaching-map.md` with eight entries across four decision families (in-season load, conditioning, session design, monitoring and standards) from eleven inspected source units, each with exact locator, ingredients, coverage note, and a checked date. `gw-seed-writer` Step 1a lane 3 runs `python -m scripts.gwqueue.evergreen_map lookup "<decision>"`; at most one archive-derived angle per run under the same gates and rubric. Forge and nightly carry the receipt rule. Runtime: `scripts/gwqueue/evergreen_map.py` validates entries, matches a decision to at most three, and lists the already-covered topics the novelty gate would flag; it never opens a source. 268 queue tests pass across F3 to F6. First real seed cycle not yet observed for any of F3 to F6. |
| **0.31.1** (2026-09-10) | **Scott's F3 decisions applied.** Freebie rule approved and live: a v1 pick's `quality:` line ends `freebie: yes` or `freebie: no` (total 17+, reader payoff 4, source support 3+); `gw-nightly-forge` obeys the line, legacy picks keep 18/20. Calibration accepted; shadow for one week; cutover 2026-09-17 by Scott's `idea-quality-settings.json`. Same night: F5 source classes approved and the first bounded pass produced ten candidate themes awaiting per-theme approval; F6 map trimmed to six entries at Scott's call. |
