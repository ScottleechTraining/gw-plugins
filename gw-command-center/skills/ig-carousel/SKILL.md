...
**Opted-in mode:** use the pre-draft Forge brief and action policy in `references/copy-record.md`. Honor an existing brief, including an explicit action reroll; do not override it with the legacy outcome menu below. If no brief exists, select automatically before writing. Batch/overnight runs never pause for an action or archetype question. The following interactive menu applies to legacy attended work. Save-led opted-in decks (new builds and explicit `action: save` rewrites) also follow `references/save-reference-slides.md`: one standalone reference body slide, a later-use CTA that names the same moment as the caption, and the phone-readability check on the rendered card. Style-only rebuilds keep the saved words even where that reference would have written something else. Share-led opted-in decks (new builds and explicit `action: share` rewrites) follow `references/share-led-decks.md` Opted-in decks also carry a teaching progression chosen by Forge from `references/rhetorical-approaches.md` When a brief carries `visual:`, that body slide follows `references/visual-teaching.md`: translate the comparison, sequence, or annotated layout into the selected pack, bind every visible string once (repeated labels separately), keep connectors landing on the intended objects, keep meaning without color, and read the rendered slide at phone size. and recorded in `brief.reason`; the builder honors it and never swaps it during a style-only rebuild.: a named recipient and joint decision in the brief, a body that carries the colleague's perspective, and a CTA and caption that name the recipient and purpose.
...
- [ ] Share-led opted-in decks (new or explicit rewrite to share): brief names sender, recipient, moment, and joint decision; no villain cover, no tag-the-coach bait; the body gives both people a next move; CTA and caption name the same recipient and purpose; a save-to-share reroll changed body, caption, and CTA together, not only the last slide.
- [ ] Opted-in decks: `brief.reason` opens with `approach: <name>`; the body follows that progression (per `references/rhetorical-approaches.md`); no first-person memory, dialogue, or admission appears unless Scott's original words back it; a style-only rebuild changed no words.
- [ ] Visual body slide (when the brief carries `visual:`): actors, step order, units, and callout targets match the saved fields; invented rows show an illustrative label; nothing decorative reads as measured data; legible at phone size; after a style-only rebuild the same relationships survive.
