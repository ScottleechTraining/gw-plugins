...
- **Type:** the cover headline is the one marker-written element: **Permanent Marker** (Google Fonts, add `family=Permanent+Marker` to the Barlow `<link>`; Scott's pick 2026-09-12 over LT Expo, which read childish), 112px, line-height 1.04, sentence case (the face is caps-only), `text-wrap: balance` per sentence. Body headlines are Vitesse Bold, 72–92px, line-height 1.1. Barlow for everything else: primary labels 44–56px Barlow 600, explanatory body 40–44px Barlow 400 at line-height 1.35–1.45, secondary labels never below 36px. Zero letter spacing. The marker face never appears on a body slide, label, takeaway, or CTA; no handwriting face as a reading font, anywhere. No dense paragraph column on a drawing slide.
...
- **Cover:** Type Plate is the default. Short headline in Permanent Marker (the coach's hand on the board), one green rule, one contained relevant real photograph below it (952 x 420, 3px ink border, B&W baked with Pillow, trap 6). Headline and image stay distinct and readable. No blur, collage, or cutout halo as the White Board default.
...
- **Status: ACTIVE.** Scott approved the rendered proofs on 2026-09-12 (`docs/superpowers/carousel-style-refresh-2026-09-12/`, Permanent Marker cover). Automatic selection may choose it per the policy below; no per-carousel question. Explicit requests and dropdown `restyle: White Board` notes work as for any pack.
...
3. **Relationship first.** If a source-supported comparison, sequence, or annotated example is the central lesson, consider White Board first and take it when its compare, sequence, or annotated layout holds that relationship clearly. If the relationship is absent, do not manufacture one; if the layout cannot hold it, fall back to Editorial.
...
| Permanent Marker | pack 7 White Board cover headline only | **0.55** | Measured 2026-09-12 in Chromium at 100px (fonts.ready awaited, real Scott-voice lines): 0.476–0.513 × font-size per character; 0.55 rounds up for punctuation. All-W probe is 0.994, so a W/M-heavy line can run ~80% wider than the budget: verify such a headline against real rendered width. Caps-only face; sentence-case input renders as caps at the same width. |
...
| White Board | `.mega-cover` (cover, Permanent Marker) | 112px | 15 | 4 | ✅ measured 2026-09-12 (952 / (0.55 × 112)); spans wrap with `text-wrap: balance`, so an over-budget sentence adds a line instead of clipping. Proof-verified: two sentences of 25 and 28 characters held four balanced lines above a 420px photo with the footer clear. Keep the two-sentence cover to 4 lines total. |
