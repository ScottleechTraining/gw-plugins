...
Used at Step 5 (toolbar + final script block). Every generated carousel ships with a Save Changes button so in-browser edits persist back to disk. It uses the File System Access API (`showSaveFilePicker` / `createWritable`); Ctrl+S (or Cmd+S) triggers it too. The first click connects the file (pick the same `.html` you are viewing); every later click saves in place. The button carries `window.__SAVE_SCRIPT_VERSION__ = '2'` so the standalone patcher (`scripts/gwqueue/patch_carousels_savebtn.py`) treats freshly-generated files as already-current and skips them.

Copy both blocks VERBATIM from below (they are sourced from the patcher) so generated and patched files are byte-identical. Do not retype or "improve" them.
...
(function() {
  ...
  window.__SAVE_SCRIPT_VERSION__ = '2';
...
The patcher skips a file only when BOTH `id="saveChangesBtn"` and `__SAVE_SCRIPT_VERSION__ = '2'` are present. Emit both blocks exactly as written.
